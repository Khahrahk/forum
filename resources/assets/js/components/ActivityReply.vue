<template>
    <activity-layout>
        <span slot="activity">
            добавил&nbsp;<a class="mr-1 text-black font-bold" :href="activity.subject.path">ответ</a>
            {{ activity.subject.created_at }} в:
        </span>

        <div slot="heading" class="text-xl font-semibold my-4">
            <a class="text-black font-bold mb-4" :href="activity.subject.thread.path">"{{ activity.subject.thread.title }}"</a>

            <p class="text-2xs text-grey-darkest font-medium mb-4">
                Запостил: <a :href="activity.subject.thread.creator.username" class="text-blue">{{
        ...
        <div slot="body">
            ...
            <div class="flex items-center py-1 text-xs text-grey-darkest">
                &#8943; <a class="ml-1 text-2xs text-blue" :href="activity.subject.path">больше</a>
            </div>
        </div>
...
<!--                </svg>-->
                ...
                <span class="text-grey-darker text-2xs font-semibold  ml-1">{{ activity.subject.favoritesCount }} Избранное</span>
            </div>
            ...
            <div v-if="activity.subject.isBest" class="text-grey-darker text-2xs font-semibold flex items-center mr-2">
                <span class="mr-2 font-bold flex items-center">
                    <span class="ml-2 mr-1">Лучший ответ!</span>
            ...
            <!-- XP Badge -->
            <div class="flex items-center w-auto h-4 px-2 bg-grey rounded font-semibold text-2xs text-white">XP +{{ activity.subject.xp }}</div>
        </div>
    </activity-layout>
</template>
